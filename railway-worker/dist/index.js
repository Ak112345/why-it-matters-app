...
const brandTemplates_1 = require("./brandTemplates");
const form_data_1 = __importDefault(require("form-data"));
fluent_ffmpeg_1.default.setFfmpegPath(ffmpeg_1.default.path);
fluent_ffmpeg_1.default.setFfprobePath(ffprobe_1.default.path);
console.log(`[worker] FFmpeg path: ${ffmpeg_1.default.path}`);
console.log(`[worker] FFprobe path: ${ffprobe_1.default.path}`);
console.log(`[worker] FFmpeg version: ${ffmpeg_1.default.version}`);
...
function extractStoragePathFromUrl(videoUrl, bucket) {
    if (!videoUrl)
        return null;
...
async function logJobEvent(jobId, level, event, details) {
    try {
...
async function updateQueueJob(jobId, patch) {
...
async function fetchPostingJob(jobId) {
...
function getFinalVideoPath(job, body) {
    if (body.final_video_path)
        return body.final_video_path;
...
function getFinalPublicUrl(finalVideoPath) {
    const { data } = supabase.storage.from('final_videos').getPublicUrl(finalVideoPath);
    return data.publicUrl;
}
async function downloadFinalVideoBuffer(finalVideoPath) {
...
function toYouTubeDescription(text) {
...
async function getYouTubeAccessToken() {
    ...
    const data = await withRetry(() => fetchJson('https://oauth2.googleapis.com/token', {
        method: 'POST',
...
async function postToYouTubeFromBuffer(videoBuffer, title, description) {
...
async function postInstagramReel(videoUrl, caption) {
    ...
    const containerData = await withRetry(() => fetchJson(`https://graph.facebook.com/v19.0/${igUserId}/media`, {
        method: 'POST',
    ...
    while (statusCode !== 'FINISHED' && attempts < 24) {
        attempts += 1;
        await sleep(5000);
        const statusData = await withRetry(() => fetchJson(`https://graph.facebook.com/v19.0/${containerId}?fields=status_code&access_token=${accessToken}`), 'instagram-container-status');
    ...
    const publishData = await withRetry(() => fetchJson(`https://graph.facebook.com/v19.0/${igUserId}/media_publish`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    let permalink;
    try {
        const mediaInfo = await withRetry(() => fetchJson(`https://graph.facebook.com/v19.0/${mediaId}?fields=permalink&access_token=${accessToken}`), 'instagram-permalink');
...
async function postFacebookVideo(videoUrl, description) {
    ...
    const accessToken = await getMetaPageToken();
    const data = await withRetry(() => fetchJson(`https://graph.facebook.com/v19.0/${pageId}/videos`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            file_url: videoUrl,
    ...
    let permalink;
    try {
        const permalinkData = await withRetry(() => fetchJson(`https://graph.facebook.com/v19.0/${postId}?fields=permalink_url&access_token=${accessToken}`), 'facebook-permalink');
...
function normalizeJobText(job, body) {
...
async function postYoutubeForJob(body) {
    const { jobId } = body;
    ...
    try {
        ...
        const youtube = await postToYouTubeFromBuffer(videoBuffer, text.title, toYouTubeDescription(text.description));
...
async function postMetaForJob(body) {
    const { jobId } = body;
    if (!jobId && !body.final_video_path && !body.videoPath) {
        throw new Error('Missing jobId or final_video_path/videoPath');
    }
...
app.get('/debug', (_req, res) => {
    ...
    const ffmpegSizeMB = (() => {
        try {
            return (fs.statSync(ffmpeg_1.default.path).size / 1024 / 1024).toFixed(1) + 'MB';
        }
        catch {
            return 'unknown';
        }
    ...
    res.json({
        status: 'ok',
        busy: isProcessing,
        ffmpegPath: ffmpeg_1.default.path,
        ffmpegVersion: ffmpeg_1.default.version,
...
async function resolvePexelsUrl(videoId) {
...
function buildDrawtextFilter(captions, fallbackHook) {
...
async function transcribeWithWhisper(filePath, startTime) {
    ...
    try {
        ...
        formData.append('file', fileStream, { filename: 'clip.mp4', contentType: 'video/mp4' });
...
function trimAndCaptionVideo(inputPath, outputPath, startTime, duration, drawtextFilter) {
...
async function saveVideoRecord(videoId, analysisId, videoUrl, finalVideoPath, hook, caption, viralityScore, durationSeconds, hasSubtitles) {
    ...
    if (finalError) {
        const missingFinalPathColumn = finalError.message.includes("Could not find the 'final_video_path' column") ||
...
async function fetchFacebookStats(postId, accessToken) {
    try {
...
async function syncPostAnalytics(jobId) {
    try {
        // Fetch job details
        ...
        const totalEngagements = (stats.youtube_likes || 0) +
        ...
        const { error: insertError } = await supabase.from('post_analytics').upsert([{ job_id: jobId, ...stats }], { onConflict: 'job_id' });
        if (insertError) {
...
app.post('/produce', requireSecret, async (req, res) => {
    ...
    const { analysisId, filePath, sourceId, startTime = 0, endTime = 10, hook, caption, explanation, contentPillar, viralityScore = 0, captions = [], } = req.body;
    ...
    try {
        ...
        const drawtextFilter = (0, brandTemplates_1.getCompleteFilterChain)({
            hook,
        ...
        await saveVideoRecord(videoId, analysisId, upload.publicUrl, upload.storagePath, hook, caption || hook, viralityScore, durationSeconds, hasSubtitles);
